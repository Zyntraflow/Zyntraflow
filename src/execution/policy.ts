...
import { hasRecentExecutionAttempt } from "./replayGuard";
...
export const evaluateExecutionPolicy = (
  ...
  cfg: ExecutionConfig,
  state: ExecutionPolicyState,
  options?: { nowMs?: number; baseDir?: string },
): ExecutionDecision => {
  ...
  if (!cfg.ALLOW_REPLAY && hasRecentExecutionAttempt(plan, cfg.REPLAY_WINDOW_SECONDS, baseDir, nowMs)) {
    return { allowed: false, reason: "REPLAY_PROTECTION_WINDOW_ACTIVE" };
  }
...
export { executionAttemptsFilePath, hasRecentExecutionAttempt, recordExecutionAttempt } from "./replayGuard";
